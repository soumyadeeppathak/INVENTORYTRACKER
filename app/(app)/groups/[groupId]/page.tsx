...
import { LocationList } from '@/src/components/locations/location-list'
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <Link href="/" className="text-gray-400 hover:text-gray-600" aria-label="Back to groups">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
            </Link>
            <span className="text-3xl" role="img" aria-label={group.name}>
              {group.emoji}
            </span>
            <h1 className="text-2xl font-bold text-gray-900">{group.name}</h1>
          </div>
          <Link href={`/groups/${groupId}/settings`}>
            <Button variant="ghost" size="sm" aria-label="Group settings">
        ...
        <LocationList groupId={groupId} locations={group.locations} />
      </div>
    )
  } catch {
    notFound()
